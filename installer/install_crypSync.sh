...
# Удаляем старый CLI, если он существует
if [ -f "$HOME/.local/bin/crypSync" ]; then
    echo "🗑️ Старый CLI найден, удаляем..."
    rm -f "$HOME/.local/bin/crypSync"
fi
