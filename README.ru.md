...
| `crypTar --server set-key` | Сменить ключ шифрования конфига |
...
Сменить GPG-ключ шифрования конфига серверов:
```bash
...
- **Ротация ключей**: для смены GPG-ключа шифрования конфига используйте `crypTar --server set-key` — он расшифрует конфиг старым ключом и атомарно перешифрует новым. Чтобы отправить публичный ключ на удалённые серверы для расшифровки архивов — используйте `crypTar --server push-key`.
